...
def CollectStat (master, sleeptime):    
    ...
    while True:
        ...
        master.totalmeshcount = 0
        master.totalmeshbandwidth = 0
        for i in range(master.rootcount):
            #print i, master.rootcount
            master.rootlocklist[i].acquire()
            master.rootlist[i].child.consolidate()
            master.rootlist[i].child.calcmesh()
            master.totalmeshcount += master.rootlist[i].child.meshcount
            master.totalmeshbandwidth += master.rootlist[i].child.meshbw
        ...
        logging.info(info)
        info = "\nTotal pull based nodes = ", master.totalmeshcount
        logging.info(info)
        info = "\nMesh rate = ", master.totalmeshbandwidth / master.totalbandwidth
        logging.info(info)
...
class Master:
    def __init__(self):
        ...
        print (self)
        _thread.start_new_thread(Consolidate, (self, 10))
        _thread.start_new_thread(CollectStat, (self, 15))
    ...
    def AddNode (self, masterdelay, delaylist, bandwidth):
        if self.rootcount < math.sqrt(self.totalnodecount + self.rootcount) and bandwidth >= ROOTBWTHRESHOLD:
    ...
    def DeleteNodeAtRandom (self):
        ...
        while True:
            ...
            if rootnode.child.total_nodes > 0:
                ...
                nid = rootnode.child.min_node.id
                if rootnode.id_to_node.get(nid,None) is not None:
                    ...
                    bw = rootnode.child.min_node.bw
                    rootnode.child.extract_min()
